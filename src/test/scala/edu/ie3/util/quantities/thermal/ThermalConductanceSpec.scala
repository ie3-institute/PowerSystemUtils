...
class ThermalConductanceSpec extends AnyFlatSpec with Matchers {
  ...
  it should "create values using UOM factories" in {
    WattsPerKelvin(1).toWattsPerKelvin shouldBe 1
  }

  it should "properly convert to all supported Units of Measure" in {
    val x = WattsPerKelvin(1)
    x.toWattsPerKelvin shouldBe 1
  }

  it should "return properly formatted strings for all supported Units of Measure" in {
    WattsPerKelvin(1).toString(WattsPerKelvin) shouldBe "1.0 W/K"
  }
