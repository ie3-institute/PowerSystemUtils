...
class IrradiationSpec extends AnyFlatSpec with Matchers {
  ...
  it should "create values using UOM factories" in {
    WattHoursPerSquareMeter(1).toWattHoursPerSquareMeter shouldBe 1
  }

  it should "properly convert to all supported Units of Measure" in {
    val x = WattHoursPerSquareMeter(1)
    x.toWattHoursPerSquareMeter shouldBe 1
  }
  ...
  it should "return Energy when multiplied by Area" in {
    WattHoursPerSquareMeter(1) * SquareMeters(10) shouldBe WattHours(10)
  }

  it should "return Irradiance when divided by Time" in {
    WattHoursPerSquareMeter(100) / Hours(10) shouldBe WattsPerSquareMeter(10)
  }
}
