...
public class FileIOUtils {
    ...
    /**
     * Compress all the files present in the provided directory (full path required! e.g. /tmp/out) or compress a
     * single file (full path + filename required! e.g. /tmp/out/sample.csv) and returns a {@link Future} with the result.
     *
     * @param dirOrFileName  path of the directory or file that should be compressed (full path required
     *                       in case of a directory / full path + filename + file extension required in case of a file!)
     * @param outputFileName the optional output filename (full path + filename + file extension
     *                       required!) if different from the provided filename
     * @param isDir          set to true if trying to compress a directory. Set to false if
     *                       compressing a single file. if set to true, will archive the files before compression
     * @return a Future containing a boolean which is either true on success or false otherwise
     * @throws FileException If unable to create the archive file, might happen, if unable to write to the output stream
     *                       or if there is some error in the input / output filename or if the archive already exists
     */
    public static CompletableFuture<Boolean> compress(final String dirOrFileName, final String outputFileName,
                                                      final boolean isDir) throws FileException {
        return CompletableFuture.supplyAsync(
                () -> {
                    try {
    ...
    /**
     * Compress all the files present in the provided directory (full path required! e.g. /tmp/out) and
    ...
    /**
     * Checks, if the output file path actually ends with '.tar.gz' or '.gz'. If not, creates a filename from the
     * directory or file name. Also, checks if the target file already exists. If it does not exist, finally creates
     * the file.
     *
     * @param dirOrFileName  path of the directory or file that should be compressed (full path required
     *                       in case of a directory / full path + filename + file extension required in case of a file!)
     ...
     */
    private static File validateOutputFileName(final String dirOrFileName, final String outputFileName, boolean isDir) throws FileException {
     ...
     */
    private static Path determineTargetDirectory(Path archive, Path target)
