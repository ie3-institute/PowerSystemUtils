...
object PricePerLength extends Dimension[PricePerLength] {
  def apply[A](n: A, unit: PricePerLengthUnit)(implicit num: Numeric[A]) = new PricePerLength(n, unit)
...
trait PricePerLengthUnit extends UnitOfMeasure[PricePerLength] with UnitConverter {
  override def apply[A](n: A)(implicit num: Numeric[A]): PricePerLength = PricePerLength(n, this)
}
